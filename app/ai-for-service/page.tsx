...
export const metadata = {
  ...
  description:
    "Empower customers with AI for Service. Agentic contact center, AI agent assistance, quality assurance, and proactive outreach. CandexAI.",
};
      ...
      <SectionWrapper variant="subtle">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <AnimateOnScroll className="mb-10">
            <h2 className="type-h2 font-semibold text-[var(--foreground)] text-center mb-4">
              The customer service challenges we solve
            </h2>
            <p className="text-[var(--muted)] text-center max-w-3xl mx-auto type-body">
              Today&apos;s contact centers are caught between rising volumes, shrinking budgets, and
              customers who expect instant, accurate resolution across every channel. Legacy AI
              failed to solve this — domain-expert agents are built specifically to.
            </p>
          </AnimateOnScroll>
          <AnimateOnScroll>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <AnimateOnScroll>
            <div className="grid md:grid-cols-3 gap-6 mb-10">
              {complianceSection.features.map((f, i) => (
                <FloatingCard
                  key={f.title}
            ...
            <Link
              href={complianceSection.cta.href}
                ...
                <div key={i} className={`solution-stat solution-stat-delay-${i + 1} text-center`}>
                  <div className="grid grid-cols-10 gap-0.5 w-[100px] mx-auto mb-4">
                    {Array.from({ length: 100 }).map((_, idx) => {
                      const fillCount = s.suffix === "x" ? (s.value as number) * 10 : s.value as number;
      ...
      <SectionWrapper>
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <AnimateOnScroll className="mb-10">
            ...
            <p className="text-[var(--muted)] text-center max-w-2xl mx-auto type-body">
              Industry-specific pre-built agent templates and use-case accelerators reduce deployment
                ...
                <ul className="space-y-3">
                  {useCasesLinks.map((u) => (
                    <li key={u.name}>
                      <Link
                        href={u.href}
                        className="flex items-center justify-between group text-[var(--foreground)] font-medium text-sm hover:text-[var(--accent)] transition-colors py-2 border-b border-[var(--border)] last:border-0"
                      >
                        {u.name}
                        <svg
